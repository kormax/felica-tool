...
/**
 * Request Service command used to verify the existence of Area and Service, and to acquire Key
 * Version
 *
 * This command is used to check if specified Areas or Services exist on the card and to retrieve
 * their Key Versions. When the specified Area or Service exists, the card returns the Key Version.
 * When it doesn't exist, the card returns 0xFFFF as Key Version.
 */
class RequestServiceCommand(
    ...
    companion object : CommandCompanion {
        override val COMMAND_CODE: Short = 0x02
        ...
        /** Parse a Request Service command from raw bytes */
        fun fromByteArray(data: ByteArray): RequestServiceCommand {
            ...
            return RequestServiceCommand(idm, nodeCodes.toTypedArray())
        }
    }
}
